...
from keras.layers import Dense, Conv2D, MaxPooling2D, Flatten, ReLU, Activation, AlphaDropout
...
def lpm(input_shape):
    ...
    x = Dense(256, activation=selu, kernel_initializer='lecun_normal')(input)
    # x = AlphaDropout(.05)(x)
    ...
    # x = AlphaDropout(.05)(x)
    x = Dense(1, kernel_initializer='lecun_normal')(x)
...
def gw(x): return np.concatenate([x.get_weights()[0].flatten(),
                                  x.get_weights()[1].flatten()], 0).flatten()
...
lpm1 = lpm(gw(d1).shape)
lpm2 = lpm(gw(d2).shape)
lpm3 = lpm(gw(d3).shape)

# Use Adadelta optimizer for sparse inputs
lpm1.compile(loss=custom_loss, optimizer=Adamax(), metrics=[get_pred])
lpm2.compile(loss=custom_loss, optimizer=Adamax(), metrics=[get_pred])
lpm3.compile(loss=custom_loss, optimizer=Adamax(), metrics=[get_pred])
...
for e in range(EPOCHS):
    for i in range(len(X_train) // BATCH_SIZE):
        ...
        loss3 = lpm3.train_on_batch(np.expand_dims(gw(d3), 0), loss)
        loss2 = lpm2.train_on_batch(np.expand_dims(
            gw(d2), 0), np.expand_dims(loss3[1], 0))
        loss1 = lpm1.train_on_batch(np.expand_dims(
            gw(d1), 0), np.expand_dims(loss2[1], 0))
        ...
        model.train_on_batch(X_train[i * BATCH_SIZE: (i+1) * BATCH_SIZE],
